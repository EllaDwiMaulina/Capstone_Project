...
export default function BuatLaporan() {
  ...
  const handleCheckLocation = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/geocode?q=${encodeURIComponent(lokasi)}`);
  ...
  const handleImageChange = (event) => {
    ...
    if (!file.type.startsWith('image/')) {
      setFormError('File harus berupa gambar JPG, PNG, atau format gambar lain.');
      event.target.value = '';
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.judul.trim() || !formData.deskripsi.trim() || !formData.kategori || !lokasi.trim()) {
      setFormError('Nama laporan, deskripsi, kategori, dan lokasi wajib diisi.');
      return;
    }
    ...
    try {
      ...
      setFormData({ judul: '', deskripsi: '', kategori: '' });
      setLokasi('');
      setMapPoint(null);
      setSelectedImage(null);
      setImagePreview('');
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="space-y-8">
            ...
            <div>
              ...
              <label className="border-2 border-dashed border-[#E5E7EB] rounded-xl min-h-[240px] p-6 flex flex-col items-center justify-center bg-[#FFFFFF] cursor-pointer hover:bg-[#F8FAFC] transition-colors overflow-hidden">
                {imagePreview ? (
                  <div className="w-full">
                    <img src={imagePreview} alt="Preview gambar laporan" className="w-full h-[180px] object-cover rounded-xl border border-[#E5E7EB]" />
                    <div className="mt-4 flex items-center justify-between gap-4">
                      <p className="text-sm text-[#1E2F4D] font-semibold truncate">{selectedImage?.name}</p>
                      <span className="text-xs text-gray-500 shrink-0">Klik untuk ganti</span>
                    </div>
                  </div>
                ) : (
                  <>
                    <Icon icon="lucide:image-plus" className="w-9 h-9 text-[#1E2F4D] mb-4" />
                    <p className="text-sm text-gray-600 text-center">
                      Klik untuk memilih gambar laporan<br />
                      <span className="text-xs">Format: JPG, PNG, WebP (Max. 5MB)</span>
                    </p>
                  </>
                )}
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="w-full h-[250px] bg-[#E5E7EB] rounded-t-xl overflow-hidden relative border-x-2 border-t-2 border-[#E5E7EB]">
                {mapPoint ? (
                  <iframe title="Preview lokasi laporan" src={mapUrl} className="w-full h-full border-0" loading="lazy" />
                ) : (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-[#1E2F4D] font-medium text-center px-4">Peta akan tampil setelah lokasi dicek</span>
                  </div>
                )}
              </div>
              <div className="w-full py-3 px-4 bg-[#FFFFFF] border-2 border-[#E5E7EB] rounded-b-xl">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <p className="text-[10px] text-[#1E2F4D] italic">
                    {mapPoint
                      ? `${mapPoint.formatted || lokasi} (${Number(mapPoint.latitude).toFixed(5)}, ${Number(mapPoint.longitude).toFixed(5)})`
            ...
            <div className="pt-8">
              <div className="relative mb-8">
                <select
                  ...
                  onChange={handleInputChange}
                  className="w-full px-4 py-4 rounded-xl border-2 border-[#E5E7EB] appearance-none outline-none focus:border-[#1E2F4D] text-gray-500 bg-[#FFFFFF]"
                >
              ...
              <button 
                ...
                className="w-full py-4 bg-[#1E2F4D] text-[#FFFFFF] rounded-xl font-bold text-xl hover:bg-[#243B63] transition-colors disabled:opacity-70"
              >
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-2xl">
            ...
            <h2 className="text-2xl font-bold text-[#1E2F4D]">Laporan Berhasil Dikirim</h2>
